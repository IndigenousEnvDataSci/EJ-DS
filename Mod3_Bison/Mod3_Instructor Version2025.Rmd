...
bison_sites <- read.csv("bison_sites.csv")
...
### Load in Biomass Data and Bison Site Datasets

Import your bison_sites and biomass datasets (download from Brightspace and then upload in Rstudio)
...
In R there are often multiple ways to do this type of coding.  You could also use the pipe operator to get a similar result:

```{r group by site}
sorted_biomass <- biomass %>%
  group_by (Site)
...
```{r group and summary} 
library(dplyr) # don't need to load in dplyr, as it is part of tidyverse which you already brought in 
...
- At this point site 3 has the highest biomass even though it has the smallest area
...
*You cannot do an accurate comparison of the sites*

## Converting the units
All of the unit conversions that need to happen: 

- Currently in $g/0.25 m^2$ and want to get to kg/ha 
- Conversion of $g/0.25 m^2$ to kg/ha is to multiply by 40
...
head(biomass2) #this is optional code that you can also add for this chunk to produce a table where we can view the first six rows of our data  
```
...
### Find the new average biomass (Students write the code to calculate this)
...
```{r new average with right units2}
avg_biomass <- biomass2 %>%   #Here I used the same code chunk to create a new variable to hold this tibble
  group_by(Site) %>% 
  summarise(avg_biomass=mean(kg_ha))
avg_biomass
...
There is another dataset the tribal agriculture managers have with the sizes of each site in hectares, called "sites", which we loaded in with biomass data at the beginning of class. We are going to merge this "bison_sites" dataset with our "biomass_avg" dataset, which has the average biomass at each site (kg per hectare), into one dataset using the function `left_join()`, which we learned in the Water Module. We can do this because both datasets have matching columns (Site). 

### Join the datasets
```{r combine datasets}
Site_biomass <- left_join(bison_sites, avg_biomass, by = "Site") #left_join(dataset 1, dataset 2, by = "shared column"); Join matching rows from dataset 2 to dataset 1 using their shared column "Site".
Site_biomass
```
...
*Now we can see the units properly.  At this point, Site 3 still has the highest biomass but it has not been adjusted for total number of hectares.*
...
*Now we can see that the total biomass of site 2 is higher because an adjustment has been made for area*
...
*Sites 2 and 3 are good candidates.*
...
#### Students, go to this site to figure out how to write your bar plot codes: [Basic barplot with ggplot2](https://r-graph-gallery.com/218-basic-barplots-with-ggplot2.html)

You may also find this site useful
[ggplot2-in-r](https://stackoverflow.com/questions/38788357/change-bar-plot-colour-in-geom-bar-with-)

#### Now, try to figure out how to add a title to your barplot (and center it) using [this site](https://stackoverflow.com/questions/40675778/center-plot-title-in-ggplot2)

#### For more advanced work: See if you can add color to your barplot!
...
library(ggplot2)
ggplot(data=max_bison, aes(x=Site, y= max_bison))+
  geom_bar(color="black", fill = "#DD8888", stat="identity")+  #stat= identity means it plots the actual values, stat=count plots actual counts
  xlab("Site") + ylab ("Number of Bison per Site")+
  ggtitle("Maximum Number of Bison per Site")+
...
```{r N group by}
dplyr::group_by(nitrogen, Site)
```

```{r total N per site}
sorted_Nitrogen <- nitrogen %>%
  group_by(Site)

head(sorted_Nitrogen)
...
nitrogen %>% 
  group_by(Site) %>% 
  summarise(avg_nitrogen=mean(Nitrogen))

```


```{r convert to kg/ha}
nitrogen2<- mutate(nitrogen, kg_ha = Nitrogen * 40)
...
nitrogen2 %>% 
  group_by (Site) %>%
  summarise(avg_nitrogen = mean(kg_ha))
...
```{r average N with right units}
avg_nitrogen <- nitrogen2 %>%
  group_by (Site) %>%
  summarise(avg_nitrogen = mean(kg_ha))
...
```{r join}
Site_nitrogen <- left_join(bison_sites, avg_nitrogen, by = "Site") #left_join(dataset 1, dataset 2, by = "shared column"); Join matching rows from dataset 2 to dataset 1 using their shared column "Site".
Site_nitrogen
...
```{r total N}
total_availableN <- mutate(Site_nitrogen, total_nitrogen = Hectares * avg_nitrogen)
total_availableN

```

```{r max bison N}
max_bisonN <- mutate(total_availableN,
                    max_bisonN=total_nitrogen/400)
max_bisonN
...
#Write code here to make a graph that visualizes the nitrogen data per site

library(ggplot2)
ggplot(data=max_bisonN,aes(x=Site, y= max_bisonN))+
  geom_bar(color="black", fill = "#DD8888", stat="identity")+  #stat= identity means it plots the                       #actual values, stat=count plots                         #actual counts
  xlab("Site") + ylab ("Max Number of Bison")+
  ggtitle("Nitrogen Consumed By Bison Per Site")+
  theme(plot.title = element_text(hjust = 0.5))
...
- Are there any other factors you would consider when looking to reintroduce a species into its native habitat?
...
able to coexist with the reintroduced bison population. Elk require 2,158 kgC/year of plant
...
```{r elk biomass per site database}

# not sure why this is here, its same as what was read in 
elk <- tibble(
  Site = c(1, 2, 3),
  Elk = c(36, 302, 209)
)
...
elk <- elk %>%
  mutate(elk_scaled = Elk*2158)%>% # consider changing elk_scaled to more descriptive variable like elk_biomass 
  select(Site, Elk, elk_scaled) # select is redundant, these are the only columns in the df 
elk

  

```
```{r join the bison and elk datasets}
#Joining the dataframes lets me get all my 
#variables in one place

bison_elk_biomass <- max_bison %>%
  left_join(elk, by = "Site") %>% 
  mutate(elk_scaled = Elk*2158) # this can be removed because it was already done above 
bison_elk_biomass
...
# Figure out how much biomass is available after elk consume some

bison_elk_biomass <- bison_elk_biomass %>% 
  mutate(biomass_difference = total_biomass - elk_scaled)
print(select(bison_elk_biomass, Site, biomass_difference))
...
# How many bison does the difference in biomass support?

names(bison_elk_biomass)

bison_elk_biomass <- bison_elk_biomass%>%
  mutate(bison_difference = biomass_difference/21900)
print(select(bison_elk_biomass, Site, bison_difference))
...
library(ggplot2) # don't need this, is loaded with tidyverse

ggplot(data=bison_elk_biomass, aes(x=Site, y= bison_difference))+
  geom_col(color="black", fill = "#DD8888", stat="identity")+   
  xlab("Site") + ylab ("Max Number of Bison - Elk")+
  ggtitle("Max Bison -Elk Consumption")+
  theme(plot.title = element_text(hjust = 0.5))

# note that geom_col does not require 'stat' but geom_bar does

# other option to plot is to have the elk and total biomass together # use colors to represent animal 
ggplot(bison_elk_biomass) + 
  geom_col(aes(x=Site, y=total_biomass, fill="remainder for bison")) +
  geom_col(aes(x=Site, y=elk_scaled, fill="biomass for elk")) +
  labs(y="total biomass kg/ha", fill="biomass allocation")
