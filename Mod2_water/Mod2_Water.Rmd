...
```{r}
stream_temp <- stream_temp %>%  #this first line of code tells R that we want to
  ...
  pivot_longer( #we use the pivot_longer code to tell R to change the columns we want
    cols = c(StreamA_temp, StreamB_temp),  #cols = tells R which columns we want to modify
...
```{r}
streams <- left_join(stream_temp,stream_DO, by="year")
...
[#JMc note to CRA: (A) I get two nuisance index columns "X.x" and "Y.y" (columns 1 and 5). (B) The site identifiers in the site.x and site.y columns have different names: "StreamA_temp" vs "StreamA_DO". We should check in at this point tomorrow.]
