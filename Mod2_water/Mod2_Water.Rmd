...
median, max, and min values for water temperature and dissolved oxygen at the two streams. 

5. NEW SKILL/review: Students will review how to make a scatter plot using ggplot. Then they will learn how to make a new kind of plot in ggplot (time series line graphs). This shows the adaptability of ggplot, and the ability to add graphical features. [Here's a link with more information](https://ggplot2.tidyverse.org/)
...
Manipulating data so that it is formatted in a way that we can easily and reliably analyze is called "data wrangling". In R, we like our data to be "tidy". This means: 

Every column of the data table is a variable. For our water quality dataset, these are categories like "temperature" and "dissolved oxygen" and also things like, "year" (which is the year the measurements were made) and "site" (which means which of the two streams was measured). 

Every row of the data table is an observation. For our water quality dataset, each row corresponds to a year where temperature and dissolved oxygen were measured at the two streams.

Every cell is a single value. For example, if row 1 is "2007" and column 2 is "temperature", the cell for row 1, column 2 might be "13.5" showing that the water temperature of a stream was 13.5 degrees C in 2007.  
...
*It will take several steps to make our data tidy.*
...
Look back at the current "messy" version of stream_temp. Can you see explain why there is more than one column with the same variable? 
...
one for the site name, and one for the dissolved oxygen values (mg/L). 
...
So far, we have separate datasets for temperature and DO. We want to combine them into one tidy dataset, so we can analyze 
...
We know that bull trout prefer cold clean water. Temperatures above 14 degrees C and dissolved oxygen levels below 6.2 mg/L  tend to be bad for bull trout, though these are only general approoximations and the healthiness of stream water for bull trout depends on many factors. 
...
Here, R is telling you that the dataset has missing data, which is indicated in R by "the "NA", which means "not available". If you look at the "streams" dataset, you'll see that there are, indeed, some NA values. 

How did these NAs get there? When we merged the separate stream_DO and stream_temp datasets using left_join(), there were years when a stream had either temperature or DO data, but not both. These are years when the K'avi were not able to measure both temperature AND dissolved oxygen at one of the streams. It is very common to have missing data in datasets. left_join() inserts "NA" into cells that don't have data.
...
In the next section, we'll go over some easy ways to fix this problem in our "water"
...
```{r}
median(na.omit(streams$temperature))
```
...
Since our goal is to compare the water quality of Stream A and Stream B, let's temporarily create separate dataframes for each stream using a new command called subset(). 

```{r}
# Below, we define a new dataframe "StreamA_WQ" as the subset of "streams" that have "StreamA" in the column "site" 
...
On Day 2 of this module, we will explore the water quality data in detail, and use this exploration of data to help inform the best decision about which stream is most promising for bull trout reintroduction. 

For now, let's use the information in the summary tables above to get a preliminary sense of which stream might have better water quality for bull trout. Comparing the two tables, which stream seems better? Which of the summary statistics is most useful? Is there any other information you'd like to know in order to make the best recommendation to the tribe?    
...
Does looking at this graph change your opinion about which stream will be healthier for bull trout? What other information would help the tribe make this decision?


**Time series graphs of water quality**

The scatter plot shows more detail than the summary statistics, but there is still more useful information in our data set. The K'avi have measured temperature and DO for 15 years, which is a long time. Since we know that temperatures are rising, it would be useful to see if these values change over time (we're looking for a trend). 

This might be particularly important for Stream B, since the water in this streams comes from melt water from a glacier that is shrinking over time. Stream A is fed by ground water, and so it might have more stable temperatures. 

These ideas form a "hypothesis" about how water quality might be changing over time: We hypothesize that the water quality of Stream A might be stable over time, while the water quality of Stream B might be getting worse.

We can assess these hypotheses by plotting water tempertature and dissolved oxygen over time. We'll make a small change to our typical ggplot() code to make a "time series" plot, which shows trends across time.
...
Do the trends over time support our hypothesis that the water quality of Stream A is stable, while the water quality of Stream B is declining? 

See if you can make a similar time-series plot showing changes in dissolved oxygen over time:
...
If you were using this analysis to make a recommendation to K'avi fisheries biologists, what would you suggest they consider as they decide which stream should be the site for bull trout reintroductions?
