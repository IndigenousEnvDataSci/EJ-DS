...
Before loading data, let's think about the reference dose (RfD) of mercury. We can use R to calculate how much mercury can safely be consumed per day based on a person's weight and fish consumption. Run the code below, and then change the `weight` and `days_per_week` variable to see how the safe amount of consumption changes. 
...
How would you describe this graph and the relationship between screening value, consumption rate, and weight? What happens to the distribution in screening values as servings per week increase? 

With this information in mind, consider how to make decisions with the data on mercury content in fish -- how should we consider the screening value for different ages? For example, what amount of servings would be safer for children, who weigh less than the average adult?
