...
1) import N dataset
2) unit conversion again (reiterate)
3) find total N (reiterate)
  - add in NAs to practice from Mod 2?
4) how many bison can have quality forage?
  - does this change the best site for reintroduction?
5) graphing
  - bar graph of N by site (reiteration if we make them do a bar graph by site for total biomass)
  - scatterpot of N against biomass--bell curve where too much biomass and too little make the plants lower quality, so middle of bell curve is best amount of biomass to maximize nitrogen (idk if anyone is doing a bell curve yet so that might be a new thing?)
