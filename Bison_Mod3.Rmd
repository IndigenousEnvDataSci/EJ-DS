...
```
Now we have the total biomass at each site in g/m^2! Unfortunately
...
```{r}
  biomass2<- mutate(biomass, kg_acre = Biomass * 16.1874257) #check that this works, need to also update this conversion to kg_hectare or kg_km2
```
** is this where we're asking them how many bison can we introduce or is this still biomass?
They will need to run the summary stats, grouped by each site, but with the new biomass column with converted units!
=======
Bison eat a lot of plants! One bison needs about 21,900 kg of biomass a year to live a healthy life. The size of the sites have also been given in hectares (ha). The dataset we have has biomass in g/m^2, which can easily be converted to match the units of kg per ha. It is important to convert units so that they are all standard across the dataset.
...
```
Now we've converted our biomass column into a new column called annual kg/ha, so that will be the new column we call on when looking at the biomass of the sites. We should take the average biomass again, but this time with the right units!

>>>>>>> Stashed changes

```{r average biomass with updated units}
biomass_avg <-biomass2 %>%
  group_by(Site) %>% 
  summarise(avg=mean(annual_kg_ha))
```

Now that we have the average annual biomass in kg/ha, this doesn't tell us exactly what we need to know about the biomass per site. It might be beneficial to total the biomass at each site. 
```{r}
biomass2 %>%
 aggregate(biomass2$annual_kg_ha, by=list(category= biomass2$Site), FUN=sum)

aggregate(annual_kg_ha ~ Site, biomass2, sum)
```
