...
4. We will challenge you to visualize the data using your previous knowledge and creativity!
...
```{r load in datasets}
biomass <- read.csv("biomass.csv", header = TRUE, sep = ',')
...
```
This new dataset--biomass2-- now has a new column that converts the biomass in g/0.25 m^2 into kg/ha. The column "kg_ha" will be the new column we will call when we are looking at the biomass data because its now in the correct units!
...
able to coexist with the reintroduced bison population. Elk require 2,518 kg/year of plant
...
If each elk requires 2,518 kg/year of plant biomass, how much biomass are the elk consuming at each site? What code would you use to figure this out?
