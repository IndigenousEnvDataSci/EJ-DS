...
* What tradeoffs are you making when you choose one of these plots over the other? 
  Potential answer: In the scatterplot the different points are more disconnected from one another so it is harder to see the trend. We don't actually have data from all ages in the line plot so the path between different points is assumed to be linear but we don't really know. 
...
* How do the different color palettes that R uses relate to the relationships between the different sites? 
  Potential answer: The color palettes with a gradient of blues assumes a relationship between the different sites where you assume similar colors will mean similar sites whereas the more "random" colors in the other palatte show that the sites are distinct. 
* Based on relationships between the different sites, what color palette would be the most appropriate here? (Note: we will talk more about changing the color palettes soon if you would like to)
  Potential answer: Since we don't really have any data on how the sites are related to one another it would probably make the most sense to use distinct colors for each site. 
