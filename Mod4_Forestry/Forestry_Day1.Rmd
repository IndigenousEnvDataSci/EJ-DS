...
Find the mean, median, maximum, and minimum of the height of the trees. 
Hint: If you're having trouble, look back at past modules.
```{r, statistics}
mean(trees$height)
median(trees$height)
max(trees$height)
...
dim(trees)
# Making a tibble
summarize(trees, mean = mean(trees$height), median= median(trees$height), maximum = max(trees$height),minimum = min(trees$height))
```


Adding a new column to the data, maybe we give them height and girth, and they
have to calculate the volume and create a new column/modify the data using mutate?


##Graph
```{r, histogram}
ggplot(trees, aes(x=height, y= age))+
  geom_point()+
